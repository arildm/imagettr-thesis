...
[TODO discuss] A function from $T_{dom}$ to $T_{rng}$ is a witness of the \textbf{function type} $(T_{dom} \rightarrow T_{rng})$.
...
[TODO discuss] Types themselves may be the witnesses of other types.
...
So, if $T = \left[\begin{array}{rcl}\text{x} &:& T'\end{array}\right]$ and $\eta = [\langle \text{x}, \text{y}\rangle]$, then $T_\eta = \left[\begin{array}{rcl}\text{y} &:& T'\end{array}\right]$.

\textbf{Flattening} transforms a nested record type into an non-nested record type.
In a nested record type such as $T = \left[\begin{array}{rcl} x &:& \left[\begin{array}{rcl} y &:& T_1 \end{array}\right] \\ z &:& T_2 \end{array}\right]$, a \textbf{path} of labels from consecutive levels can be used to address a nested field, thus $x.y$ refers to the field with the type $T_1$.
...
$\varphi(T) = \left[\begin{array}{rcl} x.y &:& T_1 \\ z &:& T_2 \end{array}\right]$
...
The \textbf{merge} of two types $T_1 \ttrmerge T_2$ is a somewhat more complicated operation.
...
\subsubsection{PyTTR: Programming with \gls{ttr}}
\label{sec:pyttr}
