...
The set of individuated objects, added to the set of relation classification results, form a set of beliefs.
...
This combination is not trivial, and is discussed in \autoref{sec:combine}.
...
% @TODO Why is ag.state.utt not String and then have ag.state.parsed : RecType?
...
It may also receive the result of parsing a natural-language utterance, upon which the utterance will be validated against the beliefs.
...
The combination of these beliefs, which declares both these relationships, is described by \autoref{eq:combination}.
To avoid conflict, new labels are created for some fields, prompting the expectation that combination results have no informative or predictable labels.
...
The result is then \textit{flattened} to avoid the nesting (\autoref{eq:combineprev}).
...
Removing duplicates does not only involve finding which fields have the same type as another field, but also relabling them so that all dependent fields refer to the same basic field as appropriate.

[why deduplication? Because relabel-subtype doesn't seem to work otherwise.]
...
\subsubsection{Translating first-order logic to TTR}
